...
class PySelenium(object):
    ...
    def get_elements(self, css):   #add by ganlp  返回数组
        """
        Judge element positioning way, and returns the element.
    ...
    def click_and_hold(self, css):  #点击左键不松开add by ganlp
        """
       Usage:
    ...
    def drag_and_drop_by_offset(self, css, xoffset, yoffset):   ##add by ganlp
        """
        Drags an element a certain distance and then drops it.

        Usage:
        driver.drag_and_drop_by_offset("id->kw",50,150)
        """
        t1 = time.time()
        try:
            self.element_wait(css)
            ...
            self.my_print("{0} Drag and drop element <{1}> to <{2},{3}>, Spend {4} seconds".format(success,
                css,xoffset,yoffset,time.time() - t1))
        except Exception:
            self.my_print("{0} Unable to drag and drop element <{1}> to <{2},{3}>, Spend {4} seconds".format(fail,
                css,xoffset,yoffset,time.time() - t1))
            raise
